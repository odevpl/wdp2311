...
class NewFurniture extends React.Component {
  ...
  renderSizeFilters() {
    ...
    const { selectedSize, activeCategory } = this.state;

    const uniqueSizes = [...new Set(products.map((item) => item.size))];

    const sizeCounts = uniqueSizes.reduce((acc, size) => {
      const count = products.filter((item) => item.size === size && item.category === activeCategory).length;
      acc[size] = count;
      return acc;
    }, {});
    ...
    return (
      <ul>
        {uniqueSizes.map((size, index) => (
          <li key={index}>
            <a
              className={`${styles.sizeFilters} ${size === selectedSize ? styles.active : ''}`}
              onClick={() => this.handleSizeChange(size)}
            >
              <span className={`${styles.checkbox} ${size === selectedSize ? styles.checked : ''}`}>
                {size === selectedSize && <span>✔</span>}
              </span>
              {size}
              <span className={`${styles.categoryCount} ${styles.sizeCount} ${size === selectedSize ? styles.active : ''}`}>
                {sizeCounts[size]}
              </span>
            </a>
          </li>
  ...
  render() {
    ...
    const { activeCategory, activePage, isFading, priceRange, selectedColor, selectedSize } = this.state;
